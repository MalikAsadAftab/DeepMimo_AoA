...
2) Absolute ULA AoA target in [0, 90]
3) Hybrid GNN:
   - auxiliary coarse angle-bin classification
   - auxiliary residual regression inside bin
   - primary direct angle regression
4) Soft decoding used consistently
5) Grouped covariance + mean complex channel node features
6) Actual selected subcarrier positions used in metadata
7) Contiguous grouping in sorted frequency order
8) MUSIC + CRB + timing + RMSE/MAE outputs

Example:
python deepmimo_gnn_grouped_hybrid_abs_ula.py \
  ...
  --cls-loss-weight 0.3 \
  --res-loss-weight 0.2 \
  --angle-loss-weight 1.0 \
  --consistency-loss-weight 0.3 \
  --outdir results_deepmimo_grouped_hybrid_abs_ula
...
def fold_aoa_to_ula_abs(angle_deg):
    """
    Fold physical azimuth in [-180, 180] to the ULA-observable absolute AoA in [0, 90]:
        theta_ula = |arcsin(sin(theta_gt))|
    """
    ang = np.asarray(angle_deg, dtype=np.float64)
    folded = np.rad2deg(np.arcsin(np.sin(np.deg2rad(ang))))
    return np.abs(folded).astype(np.float32)


def angular_error_deg(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
    """
    Generic wrapped angular error for signed angles.
    """
...
def make_angle_bins(num_bins: int, min_deg: float = 0.0, max_deg: float = 90.0):
    if num_bins < 2:
...
def class_and_residual_to_angle_deg(
    ...
    out = center_vals + residual_deg
    return np.clip(out, 0.0, 90.0).astype(np.float32)
...
def soft_angle_from_logits_and_residual(
    ...
    pred_angle = torch.clamp(pred_angle, 0.0, 90.0)
    return pred_angle
...
def approx_grouped_crb_deg(
    ...
    """
    Approximate CRB for the absolute ULA angle.
    Still an approximation and should be interpreted cautiously.
    """
    M = h_sample.shape[0]
...
def load_deepmimo_candidate_channels(
    ...
    if fold_labels_to_ula:
        labels_deg = fold_aoa_to_ula_abs(labels_deg_raw)
    else:
...
def make_contiguous_groups_from_selected(selected_rel_idx, group_size):
    selected_sorted = np.array(sorted(selected_rel_idx), dtype=int)
    groups_abs = []
    for i in range(0, len(selected_sorted), group_size):
        groups_abs.append(selected_sorted[i:i + group_size].tolist())
    return groups_abs
...
def select_subcarriers_and_groups(
    ...
    # revised: group in sorted frequency order, not similarity clusters
    groups_sc_candidate = make_contiguous_groups_from_selected(selected_rel_idx_sorted, group_size)
...
def channel_to_graph_grouped(
    ...
    centers: np.ndarray,
    bin_width: float,
    selected_rel_idx: np.ndarray,
    total_candidate_subc: int,
    topk: int = 0
) -> Data:
    ch_selected = normalize_complex_channel(ch_selected)
    selected_rel_idx = np.asarray(selected_rel_idx)
    ...
    for g in groups_sc:
        ...
        # richer direct summary
        h_mean = np.mean(hg, axis=1)
        feat_mean = np.concatenate([np.real(h_mean), np.imag(h_mean)], axis=0).astype(np.float32)

        actual_sc = selected_rel_idx[np.array(g)]
        center_sc = float(np.mean(actual_sc) / max(total_candidate_subc - 1, 1))
        width_sc = float((np.max(actual_sc) - np.min(actual_sc) + 1) / max(total_candidate_subc, 1))
        feat_meta = np.array([center_sc, width_sc], dtype=np.float32)

        feat = np.concatenate([feat_cov, feat_mean, feat_meta], axis=0)
    ...
    for i in range(len(labels_deg)):
        dataset.append(
            channel_to_graph_grouped(
                ...
                centers=centers,
                bin_width=bin_width,
                selected_rel_idx=selected_rel_idx,
                total_candidate_subc=total_candidate_subc,
                topk=topk
            )
        )
    return dataset
...
class HybridAoAGNN(nn.Module):
    ...
    def forward(self, data):
        ...
        angle_pred = 90.0 * torch.sigmoid(self.angle_head(z)).squeeze(-1)
        return cls_logits, residual_norm, angle_pred
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        cls_logits, residual_norm, angle_pred = model(batch)
        ...
        loss_ang = F.smooth_l1_loss(angle_pred, target_angle)

        pred_angle_soft = soft_angle_from_logits_and_residual(
            cls_logits, residual_norm, centers, bin_width, device
        )
        loss_consistency = F.smooth_l1_loss(pred_angle_soft, angle_pred)

        loss = (
            cls_loss_weight * loss_cls
            + res_loss_weight * loss_res
            + angle_loss_weight * loss_ang
...
def evaluate_gnn(
    ...
    for batch in loader:
        ...
        pred_class, pred_res, pred_angle_soft_np = decode_predictions_soft(
            cls_logits, residual_norm, centers, bin_width, device
        )
        pred_angle_main_np = angle_pred.detach().cpu().numpy().astype(np.float32)

        all_true_angle.append(target_angle.cpu().numpy())
        all_true_cls.append(target_cls.cpu().numpy())
        all_pred_cls.append(pred_class)
        all_pred_angle_soft.append(pred_angle_soft_np)
        all_pred_angle_main.append(pred_angle_main_np)
    ...
    y_pred_angle_soft = np.concatenate(all_pred_angle_soft)
    y_pred_angle_main = np.concatenate(all_pred_angle_main)

    # use direct angle regression as main output
    y_pred_angle = y_pred_angle_main
    ...
    return y_true_angle, y_pred_angle, {
        ...
        "class_acc": class_acc,
        "mae": mae_deg(y_true_angle, y_pred_angle),
        "rmse": rmse_deg(y_true_angle, y_pred_angle),
        "mae_soft": mae_deg(y_true_angle, y_pred_angle_soft),
        "rmse_soft": rmse_deg(y_true_angle, y_pred_angle_soft),
...
def run_music_dataset_grouped(
    ...
    for i in range(ch_bs_sc.shape[0]):
        ...
        t0 = time.perf_counter()
        pred_signed, Pagg = run_music_single_sample_grouped(
            h_sample=h,
            groups_sc=groups_sc,
            num_sources=1,
        ...
        pred_abs = abs(pred_signed)

        preds_signed.append(pred_signed)
        ...
        try:
            crb_deg = approx_grouped_crb_deg(
                h_sample=h,
                theta_deg=float(labels_deg_abs[i]),
                groups_sc=groups_sc,
                d_over_lambda=0.5,
                num_sources=1
            )
    ...
    preds_signed = np.asarray(preds_signed, dtype=np.float32)
    preds_abs = np.asarray(preds_abs, dtype=np.float32)
    ...
    return preds_abs, preds_signed, spectra_all, crb_deg_all, {
        "mae": mae_deg(labels_deg_abs, preds_abs),
        "rmse": rmse_deg(labels_deg_abs, preds_abs),
        "infer_time_total_sec": total_time,
        "infer_time_per_sample_sec": total_time / max(len(preds_abs), 1),
...
def plot_training_curve(train_losses, val_mae, val_rmse, val_class_acc, outpath):
...
def run_one_config(
    ...
    all_graphs = build_graph_dataset_grouped(
        ...
        bin_width=bin_width,
        selected_rel_idx=selected_rel_idx,
        total_candidate_subc=args.candidate_subc,
        topk=args.topk
    )
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            ...
            res_loss_weight=args.res_loss_weight,
        ...
        print(
            ...
            f"Val MAE soft={val_metrics['mae_soft']:.4f} | "
            f"Val RMSE soft={val_metrics['rmse_soft']:.4f}"
        )
    ...
    y_pred_music_abs, y_pred_music_signed, spectra_all, crb_deg_all, music_metrics = run_music_dataset_grouped(
        ch_bs_sc=test_ch_selected,
        labels_deg_abs=test_labels,
        groups_sc=groups_sc,
        angle_grid_deg=angle_grid_deg
    )

    rows = []
    gnn_err = y_pred_gnn - test_labels
    music_err = y_pred_music_abs - test_labels

    for i in range(len(test_labels)):
        rows.append({
            "sample": int(i),
            "gt_doa_abs": float(test_labels[i]),
            "gnn_doa_abs": float(y_pred_gnn[i]),
            "music_doa_abs": float(y_pred_music_abs[i]),
            "music_doa_signed_raw": float(y_pred_music_signed[i]),

            "gnn_abs_error": float(abs(gnn_err[i])),
            "music_abs_error": float(abs(music_err[i])),
            "gnn_sq_error": float(gnn_err[i] ** 2),
            "music_sq_error": float(music_err[i] ** 2),
    ...
    for i in range(min(len(test_labels), args.max_plot_samples)):
        ...
        gt = test_labels[i]
        gnn = y_pred_gnn[i]
        music_abs = y_pred_music_abs[i]
        music_signed = y_pred_music_signed[i]
        ...
        plt.axvline(-gt, linestyle="-", linewidth=1.5, alpha=0.5, label=f"GT=-{gt:.1f}°")
        plt.axvline(music_signed, linestyle="--", linewidth=2, label=f"MUSIC raw={music_signed:.1f}°")
        plt.axvline(+gnn, linestyle=":", linewidth=2.5, label=f"GNN |AoA|={gnn:.1f}°")
        plt.xlabel("Angle (degree)")
        plt.ylabel("Spectrum (normalized)")
        plt.title(
            f"Test sample {i} | {selection_method} | sel={n_select} | g={group_size}\n"
            f"MUSIC abs err={abs(music_abs - gt):.2f}°, GNN abs err={abs(gnn - gt):.2f}°"
        )
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
    ...
    plt.plot(test_labels[:num_plot], label="GT |AoA|")
    plt.plot(y_pred_music_abs[:num_plot], label="MUSIC |AoA|")
    plt.plot(y_pred_gnn[:num_plot], label="GNN |AoA|")
    ...
    run_summary = {
        ...
        "gnn_mae_soft": gnn_metrics["mae_soft"],
        "gnn_rmse_soft": gnn_metrics["rmse_soft"],
...
def main():
    ...
    parser.add_argument("--group-list", type=str, default="4,5,10")
    ...
    parser.add_argument("--num-angle-bins", type=int, default=19)

    parser.add_argument("--cls-loss-weight", type=float, default=0.3)
    parser.add_argument("--res-loss-weight", type=float, default=0.2)
    parser.add_argument("--angle-loss-weight", type=float, default=1.0)
    parser.add_argument("--consistency-loss-weight", type=float, default=0.3)

    parser.add_argument("--max-plot-samples", type=int, default=30)
    parser.add_argument("--max-doa-curve-samples", type=int, default=300)

    parser.add_argument("--outdir", type=str, default="results_deepmimo_grouped_hybrid_abs_ula")
    ...
    centers, bin_width = make_angle_bins(args.num_angle_bins, 0.0, 90.0)
        ...
        for nsel in n_select_list:
            for gsize in group_values:
                ...
                final_table_rows.append({
                    ...
                    "GNN MAE soft": summ["gnn_mae_soft"],
                    "GNN RMSE soft": summ["gnn_rmse_soft"],
    ...
    for method in methods:
        ...
        for gsize in sorted(sdf["group_size"].unique()):
            ...
            plt.plot(sdfg["n_select"], sdfg["gnn_mae"], label=f"{method} g={gsize} GNN")
            plt.plot(sdfg["n_select"], sdfg["music_mae"], label=f"{method} g={gsize} MUSIC")
    plt.xlabel("n_select")
    plt.ylabel("MAE (deg)")
    plt.title("Absolute ULA MAE vs n_select")
    plt.grid(True)
    ...
    for method in methods:
        ...
        for gsize in sorted(sdf["group_size"].unique()):
            sdfg = sdf[sdf["group_size"] == gsize].sort_values("n_select")
            plt.plot(sdfg["n_select"], sdfg["gnn_rmse"], label=f"{method} g={gsize} GNN")
            plt.plot(sdfg["n_select"], sdfg["music_rmse"], label=f"{method} g={gsize} MUSIC")
    plt.xlabel("n_select")
    plt.ylabel("RMSE (deg)")
    plt.title("Absolute ULA RMSE vs n_select")
    plt.grid(True)
    plt.legend(fontsize=7, ncol=2)
    plt.tight_layout()
    plt.savefig(os.path.join(args.outdir, "comparison_rmse.png"), dpi=200)
    plt.close()
