...
"""
DeepMIMO + Grouped MUSIC + GNN sweep
Improved version:
- DeepMIMO GT azimuth folded to ULA-observable [-90, 90]
- MUSIC evaluated correctly for ULA sign ambiguity
- GNN improved with:
    * sin/cos target instead of direct scalar angle
    * covariance-based grouped node features
    * group center / width metadata
    * GATv2 + residual + LayerNorm
- Saves both:
    * signed-angle metrics
    * ULA-abs-angle metrics

Example:
python deepmimo_gnn_grouped_sweep_improved.py \
  --scenario asu_campus_3p5 \
  --candidate-subc 64 \
  --subc-start 0 \
  --n-select-list 20,30,40,50,60 \
  --group-list 5,10 \
  --run-all-methods \
  ...
  --topk 2 \
  --trace-norm \
  --outdir results_deepmimo_grouped_improved
...
def angular_error_deg(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
    """
    Signed circular error in [-180, 180).
    """
...
def ula_abs_error_deg(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
    """
    ULA-appropriate ambiguity-free error:
        ||theta_true| - |theta_pred||
    """
    y_true = np.asarray(y_true, dtype=np.float64)
    y_pred = np.asarray(y_pred, dtype=np.float64)
    return np.abs(np.abs(y_true) - np.abs(y_pred))
...
def sincos_to_angle_deg(y_sincos: np.ndarray) -> np.ndarray:
    y_sincos = np.asarray(y_sincos, dtype=np.float64)
    ang = np.rad2deg(np.arctan2(y_sincos[:, 0], y_sincos[:, 1]))
...
def run_music_single_sample_grouped(
    ...
    groups_sc: List[List[int]],        # local indices inside selected subcarriers
    num_sources: int = 1,
    angle_grid_deg: Optional[np.ndarray] = None
):
    if angle_grid_deg is None:
...
def load_deepmimo_candidate_channels(
    ...
    Returns:
        channels_candidate
        labels_deg: folded to [-90, 90] for ULA if requested
        meta
    """
    try:
...
def channel_to_graph_grouped(
    ...
    for g in groups_sc:
        ...
        feat_cov = np.concatenate([
            np.real(Rg).reshape(-1),
            np.imag(Rg).reshape(-1)
    ...
    theta_rad = np.deg2rad(label_deg)
    y = np.array([np.sin(theta_rad), np.cos(theta_rad)], dtype=np.float32)
...
class GNNRegressor(nn.Module):
    ...
    def forward(self, data):
        ...
        x = self.pool(x, batch)
        out = self.head(x)
        out = F.normalize(out, p=2, dim=-1)   # force onto unit circle
        return out
...
def evaluate_gnn(model, loader, device):
    ...
    for batch in loader:
        ...
        ys_sincos.append(batch.y.view(-1, 2).cpu().numpy())
        preds_sincos.append(pred.cpu().numpy())

        total_time += (t1 - t0)
        total_n += batch.y.view(-1, 2).shape[0]

    ys_sincos = np.concatenate(ys_sincos, axis=0)
    preds_sincos = np.concatenate(preds_sincos, axis=0)

    ys_deg = sincos_to_angle_deg(ys_sincos)
    preds_deg = sincos_to_angle_deg(preds_sincos)

    return ys_deg, preds_deg, {
        "mae_signed": mae_deg(ys_deg, preds_deg),
        "rmse_signed": rmse_deg(ys_deg, preds_deg),
        "mae_ula": mae_ula_deg(ys_deg, preds_deg),
        "rmse_ula": rmse_ula_deg(ys_deg, preds_deg),
...
def plot_training_curve(train_losses, val_maes_signed, val_maes_ula, outpath):
    plt.figure(figsize=(8, 5))
    plt.plot(train_losses, label="Train Loss")
    plt.plot(val_maes_signed, label="Val MAE Signed")
    plt.plot(val_maes_ula, label="Val MAE ULA")
    plt.xlabel("Epoch")
    plt.ylabel("Metric")
...
def run_one_config(
    ...
    for i in range(min(len(test_labels), args.max_plot_samples)):
        ...
        plt.title(
            ...
            f"MUSIC ULA err={abs(abs(gt)-abs(music)):.2f}°, "
            f"GNN signed err={abs(((gnn-gt+180)%360)-180):.2f}°"
        )
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="results_deepmimo_grouped_improved")
    ...
    plt.xlabel("n_select")
    plt.ylabel("MAE (deg)")
    plt.title("ULA-Abs MAE vs n_select")
    plt.grid(True)
    plt.legend(fontsize=7, ncol=2)
    plt.tight_layout()
