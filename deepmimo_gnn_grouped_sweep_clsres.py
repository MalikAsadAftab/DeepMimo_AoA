...
1) ULA-aware MUSIC evaluation
2) GNN with coarse angle-bin classification + residual regression
3) Covariance-based grouped node features
4) Group center/width metadata
5) GATv2 / SAGE support with residual connections and LayerNorm

Key ideas:
- GT azimuth is folded to ULA-observable range [-90, 90]
- MUSIC is evaluated both with signed error and ULA-abs error
- GNN predicts:
    * coarse class over angle bins
    * fine residual inside the predicted bin
- Final GNN angle = bin_center + residual
- Residual is constrained to half-bin width

Example:
python deepmimo_gnn_grouped_sweep_clsres.py \
  ...
  --outdir results_deepmimo_grouped_clsres
...
class GNNClsResRegressor(nn.Module):
    def __init__(
        self,
        in_dim: int,
        num_bins: int,
    ...
    def forward(self, data):
        ...
        residual_norm = torch.tanh(self.res_head(z)).squeeze(-1)  # [-1, 1]
...
def train_one_epoch(model, loader, optimizer, device, centers, bin_width, class_loss_weight=1.0, residual_loss_weight=1.0):
    model.train()
    total_loss = 0.0
    total_n = 0
    ...
    for batch in loader:
        ...
        loss = class_loss_weight * loss_cls + residual_loss_weight * loss_res
        loss.backward()
        optimizer.step()
...
@torch.no_grad()
def evaluate_gnn(model, loader, device, centers, bin_width, class_loss_weight=1.0, residual_loss_weight=1.0):
    model.eval()
    ...
    for batch in loader:
        ...
        pred_class, pred_res, pred_angle = decode_predictions(cls_logits, residual_norm, centers, bin_width)

        all_true_angle.append(batch.y_angle.view(-1).cpu().numpy())
...
def run_one_config(
    ...
    for i in range(min(len(test_labels), args.max_plot_samples)):
        ...
        plt.title(
            ...
            f"GNN ULA err={abs(abs(gt)-abs(gnn)):.2f}°"
        )
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="results_deepmimo_grouped_clsres")
